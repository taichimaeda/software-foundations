...
Proof.
  ...
  (* - trivial.
  - simpl. rewrite -> add_0_r. rewrite <- plus_n_Sm. rewrite -> add_0_r. reflexivity.
  - simpl. rewrite -> ? add_0_r. rewrite -> IHb. rewrite -> plus_n_Sm. rewrite -> add_assoc. rewrite -> (add_comm _ 1). rewrite -> add_assoc. simpl (1 + 1). rewrite -> add_comm. rewrite -> (add_comm 2 _). rewrite -> add_assoc. reflexivity. *)
  - trivial.
  - simpl. lia.
...
Proof.
  ...
  induction n as [| n' IHn'].
  - reflexivity.
  - simpl. rewrite -> bin_to_nat_pres_incr. lia.
Qed.
...
Proof.
  intros n.
  induction n as [|b IHb|b IHb]; reflexivity.
Qed.
...
(*
Because B0 (B0 Z) and B0 Z are equivalent, but not equal.
*)
...
Lemma nat_to_bin_pres_incr : forall n, nat_to_bin (S n) = incr (nat_to_bin n).
Proof.
  intros n. reflexivity.
Qed.

Lemma nat_to_bin_double_bin : forall n, nat_to_bin (n + n) = double_bin (nat_to_bin n).
Proof.
  intros n.
  induction n as [|n IHn].
  - reflexivity.
  - simpl. rewrite -> double_incr_bin. rewrite <- IHn. rewrite <- plus_n_Sm. rewrite -> nat_to_bin_pres_incr. reflexivity.
Qed.

Lemma nat_to_bin_incr : forall n, nat_to_bin (n + 1) = incr (nat_to_bin n).
Proof.
  intros n.
