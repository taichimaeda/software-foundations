...
Definition sum : bag -> bag -> bag := fun x y => x ++ y.
...
Theorem add_inc_count : forall v s, count v (v :: s) = S (count v s).
Proof.
...
Theorem count_member_nonzero : forall (s : bag),
  1 <=? (count 1 (1 :: s)) = true.
Proof.
  intros s. induction s as [|n s' IHs']; reflexivity.
Qed.
(** [] *)
...
Proof.
  intros s1 s2 n. induction s1 as [|m s1' IHs1'].
  - reflexivity.
  - simpl. destruct (m =? n).
...
Proof.
  intros f H n1 n2 Hf. rewrite -> (H n1). rewrite -> (H n2).
  f_equal. exact Hf.
Qed.
...
Proof.
  intros l d. destruct l; reflexivity.
Qed.
(** [] *)
...
Proof.
  intros x. destruct x as [n]. simpl. rewrite -> eqb_refl. reflexivity. 
Qed.
(** [] *)
...
Proof.
  intros d x v.
  destruct x. simpl. rewrite -> eqb_refl. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros d x y o H.
  destruct y. simpl. rewrite -> H. reflexivity.
Qed.
(** [] *)
End PartialMap.
