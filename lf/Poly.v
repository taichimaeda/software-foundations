...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  match l with
  | [] => ([], [])
  | (a, b) :: t => let (lx, ly) := split t in (a :: lx, b :: ly)
  end.
...
Proof.
  ...
  - reflexivity.
  - simpl. rewrite <- IHl'.
    rewrite -> map_app_distr. reflexivity.
Qed.
(** [] *)
...
(* 
When counting the number of true in a boolean list, X is bool and Y is nat.
    [] *)
...
*)
(*
Proof by induction length n of list l.
*)
...
(*
(n X) f is a function that applies f n times
m (X -> X) (n X) is a function that applies (n X) m times
m (X -> X) (n X) f is a function that applies f (n^m) times
*)
